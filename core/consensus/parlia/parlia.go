...
import (
	...
	"github.com/FinanceFutureFactory/3fcoin/config"
	...
	ethereum "github.com/FinanceFutureFactory/3fcoin"
	"github.com/FinanceFutureFactory/3fcoin/core/accounts"
	"github.com/FinanceFutureFactory/3fcoin/core/accounts/abi"
	"github.com/FinanceFutureFactory/3fcoin/core/common"
	"github.com/FinanceFutureFactory/3fcoin/core/common/gopool"
	"github.com/FinanceFutureFactory/3fcoin/core/common/hexutil"
	"github.com/FinanceFutureFactory/3fcoin/core/consensus"
	"github.com/FinanceFutureFactory/3fcoin/core/consensus/misc"
	"github.com/FinanceFutureFactory/3fcoin/core/core"
	"github.com/FinanceFutureFactory/3fcoin/core/core/forkid"
	"github.com/FinanceFutureFactory/3fcoin/core/core/state"
	"github.com/FinanceFutureFactory/3fcoin/core/core/systemcontracts"
	"github.com/FinanceFutureFactory/3fcoin/core/core/types"
	"github.com/FinanceFutureFactory/3fcoin/core/core/vm"
	"github.com/FinanceFutureFactory/3fcoin/core/crypto"
	"github.com/FinanceFutureFactory/3fcoin/core/ethdb"
	"github.com/FinanceFutureFactory/3fcoin/core/lib/ethapi"
	"github.com/FinanceFutureFactory/3fcoin/core/log"
	"github.com/FinanceFutureFactory/3fcoin/core/params"
	"github.com/FinanceFutureFactory/3fcoin/core/rlp"
	"github.com/FinanceFutureFactory/3fcoin/core/rpc"
	"github.com/FinanceFutureFactory/3fcoin/core/trie"
)
...
func New(
	...
	vABI2, err := abi.JSON(strings.NewReader(stakeABI))
	if err != nil {
		panic(err)
	}
...
func (p *Parlia) Seal(chain consensus.ChainHeaderReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	...
	log.Info("Sealing block with", "number", number, "delay", delay, "headerDifficulty", header.Difficulty, "val", val)
...
func (p *Parlia) distributeIncoming(val common.Address, state *state.StateDB, header *types.Header, chain core.ChainContext,
	...
	method := "GetAllStakeInfo"

	amount := big.NewInt(0)
	// get packed data
	data, err := p.stakeABI.Pack(method)
	if err != nil {
	...
	for k := range ret0 {
		stakeReward := new(big.Int).Div(new(big.Int).Mul(config.GetBlockCoinbaseReward(val.Hex()), ret0[k].StakeCount), sumStake)
	...
	if header.Number.Int64() > 0 && new(big.Int).Mod(header.Number, config.MintBlockNum).Cmp(big.NewInt(0)) == 0 && header.Number.Cmp(big.NewInt(100000000)) <= 0 { //能够被整除，且小于1亿个区块
		state.AddBalance(common.HexToAddress(config.MintAddress), config.MintBlockReward) //增发奖励
	}
	return nil
}

func (p *Parlia) getCurrStakeFFF(val common.Address, state *state.StateDB, header *types.Header, chain core.ChainContext,
	txs *[]*types.Transaction, receipts *[]*types.Receipt, receivedTxs *[]*types.Transaction, usedGas *uint64, mining bool) *big.Int {
	...
	data, err := p.stakeABI.Pack(method)
	if err != nil {
		log.Error("Unable to pack tx for currStakePeople", "error", err)
		return nil
	}
	// get system message
...
func (p *Parlia) getStakeInfoIndexMap(index int64, state *state.StateDB, header *types.Header, chain core.ChainContext,
	txs *[]*types.Transaction, receipts *[]*types.Receipt, receivedTxs *[]*types.Transaction, usedGas *uint64, mining bool) (common.Address, error) {

	method := "stakeInfoIndexMap"
	stakeIndex := big.NewInt(index)
	// get packed data
	data, err := p.stakeABI.Pack(method, stakeIndex)
	if err != nil {
		log.Error("Unable to pack tx for stakeInfoIndexMap", "error", err)
	...
	return common.BytesToAddress(buff), err

}

func (p *Parlia) getStakeInfoMap(index int64, state *state.StateDB, header *types.Header, chain core.ChainContext,
	txs *[]*types.Transaction, receipts *[]*types.Receipt, receivedTxs *[]*types.Transaction, usedGas *uint64, mining bool) (common.Address, error) {
...
func (p *Parlia) initContract(state *state.StateDB, header *types.Header, chain core.ChainContext,
	...
	for _, c := range contracts {
		...
		log.Trace("init contract", "block hash", header.Hash(), "contract", c, "err", err)
		if err != nil {
			return err
		}
	}
...
// slash spoiled validators
func (p *Parlia) distributeToStaker(amount *big.Int, state *state.StateDB, header *types.Header, chain core.ChainContext,
	txs *[]*types.Transaction, receipts *[]*types.Receipt, receivedTxs *[]*types.Transaction, usedGas *uint64, mining bool) error {
	// method
	...
	if err != nil {
		log.Error("Unable to pack tx for DistributeBlockReward", "error", err)
		return err
	}
...
// get system message
func (p *Parlia) getSystemMessage2(from, toAddress common.Address, data []byte, value *big.Int) ethereum.CallMsg {
