...
// and holds the token for the program.
...
// Feed feeds token in to ch and are interpreted by
// the compiler.
//
...
// Compile compiles the current token and returns a
// binary string that can be interpreted by the EVM
// and an error if it failed.
//
// compile is the second stage in the compile phase
// which compiles the token to EVM instructions.
func (c *Compiler) Compile() (string, []error) {
	var errors []error
	// continue looping over the token until
