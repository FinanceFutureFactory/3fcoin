...
// the token channels of the lexer
type token struct {
	typ    tokenType
	lineno int
	text   string
...
// source code and turning them in to token.
// Tokens are interpreted by the compiler.
type lexer struct {
	input string // input contains the source code of the program

	tokens chan token // token is used to deliver token to the listener
...
// channel on which the token are delivered.
